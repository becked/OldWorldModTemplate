...
## 2026-09-09 Update (Update #150, v1.0.84658)

XML, Reference/Source C#, and decompiled changes across 11 XML files (14 modified, 3 added, 0 removed records), 40 Reference/Source C# files, and ~33 decompiled files. Highlights: **Forts can no longer be built adjacent to other Forts**, an **AI build-mix overhaul** that counts latest-tech and obsolete land units separately (fewer Militia, Conscripts and obsolete builds), a new **order-surplus budget** gating AI expansion attacks, a per-unit **"moved by an ally" lock**, **mirrored map scripts locking an outer water margin**, **council rating yields now applying the yield's triangle offset** (cutting Ambassador/Spymaster/Chancellor culture and science), four **new data-driven `InfoImprovement` fields** for modders, and a temporary **Mac crash debug harness** gated to developer machines. Unity engine stays at 6000.3.15f1 (verified in `UnityPlayer.dylib`/asset bundle).

### Gameplay

- **Forts cannot be built adjacent to other Forts** — `improvementClass.xml`: `IMPROVEMENTCLASS_FORT` `bNoAdjacent (absent)→1`; the class has exactly one member, `IMPROVEMENT_FORT`. The `mbNoAdjacent` reader is pre-existing — what changed alongside it is the help plumbing in `HelpText.buildImprovementRequiresHelp`, which splits the rule between the type-context (`pTile == null`, new `TEXT_HELPTEXT_IMPROVEMENT_REQUIRES_NO_ADJACENT` requirement line) and the tile-context, where the warning moved to *after* the `mbAdjacentValid` check and is now gated on `!bAdjacencyValid`
- **Garrison line gains 10% friendly defense at every tier** — `improvement.xml`: `IMPROVEMENT_GARRISON_1` (Garrison), `_2` (Stronghold), `_3` (Citadel) `iDefenseModifierFriendly 20/30/40→30/40/50`
- **Council rating yields now apply the yield's triangle offset** — `InfoHelpers.getRatingYieldRateCouncil` passes `mInfos.yield(eYield).miTriangleOffset` instead of a literal `0`, matching what `getRatingYieldRateCourt` already did; `yield.xml`: `YIELD_CULTURE` `iTriangleOffset (absent)→-2` (`YIELD_SCIENCE` already carried `-2` and is unchanged). Together these cut the culture and science that Ambassador, Spymaster and Chancellor derive from ratings — `Utils.triangleOffset(n, off)` returns `sign(n) * (triangle(|n| + off) - off)`, so a rating of 3 drops from 6 to 3
- **Farms are invalid on water targets** — `improvement.xml`: `IMPROVEMENT_FARM` `TerrainInvalid (absent)→TERRAIN_TARGET_WATER`
- **Alliance ambitions no longer offered below three teams** — `Player.isGoalPossible` (`PlayerGoal.cs`) adds an `infos().Globals.TEAM_ALLIANCE_STAT` branch returning false when `game().getNumTeamsInt() <= 2`
- **Trashed techs are no longer auto-passed on culture growth** — `Player.onCultureGrow` adds `&& !isTechTrashed(eLoopTech)` to the guard before `setTechPassed(eLoopTech, true)`
- **City effects can be summed without their chained sub-effects** — `City.addEffectYield` gains a trailing `bool bBaseOnly = false` that wraps its `maeEffectCityEffectCity` chained-effect loop in `if (!bBaseOnly)`; `Tile.yieldOutputCityEffects` is the sole caller passing `true` (the double-counting fix for previews and help text)
- **Hotseat games stay live through the tribe turn** — `Game.updateServer`'s `isMultiplayerHotseat()` branch adds `if (isTribeTurn()) { bGameOverAndDone = false; }` ahead of the per-player loop
- **City-site settlement improvement restored through the load path** — `Game.postStart` changes `pTile.setImprovementFinished(...)` → `pTile.loadImprovement(infos().Globals.CITY_SITE_IMPROVEMENT)` for active city sites with no improvement
- **Losing-side event trigger no longer fires for the winners** — `Game.makeGameOver` wraps the `LOSE_GAME_EVENTTRIGGER` call in `if (!isWinnerTeam(pLoopPlayer.getTeam()))`
- **Victory-completion triggers are restricted to visible stories** — `Player.doEventTrigger` (`PlayerEvent.cs`) gains a trailing `bool bVisibleOnly = false`; both `bFound = true` paths now require `!infos().eventStory(...).mbHidden` when set. `Game.doVictory` is the only caller passing `bVisibleOnly: true`
- **Learn to Play demo games run 60 turns instead of 40** — `LearnToPlay1Game` through `LearnToPlay5Game` each add an identical `getMaxTurns()` override returning `60` under `isDemo()`; the base `Game.getMaxTurns` returns `40` and is unchanged
- **Chop Wood goal eased in Rise of Carthage 2** — `Mods/Carthage2/Infos/text-eventOption-add.xml`: `TEXT_EVENTOPTION_CARTHAGE2_LAUNCH_GOAL_2` "Chop Wood twenty times"→"fifteen times"

### Combat

- **Polybolos attack bonus made unconditional** — `effectUnit.xml`: `EFFECTUNIT_RAPID_FIRE` drops `aiUnitTraitModifierAttack.UNITTRAIT_RANGED 50` in favour of a flat `iAttackModifier (absent)→50`; the effect is carried by `UNIT_POLYBOLOS`
- **Allied shared vision moved from query time to bookkeeping time** — `Tile.getVisibility` loses its `game().hasTeamAlliance(eIndex)` early-return for `VISIBLE`; instead `Tile.changeVisibleCountAdjacent` gains a `bool bDoAlly = true` parameter and, when set, recurses once onto `game().getTeamAlliance(eIndex)` with `bDoAlly: false`, so the ally's visible counts are incremented directly rather than inferred on lookup
- **Improvements can hide units standing on them** — `Unit.hideTerrainTarget` returns true up front when `pTile.hasImprovement() && pTile.improvement().mbHideUnits`, ahead of the existing `EffectUnit`/`mabHideTerrainTarget` scan

### AI

- **Build mix now counts latest-tech and obsolete land units separately** — new `PlayerAI.getCurrentAllLandMilitaryUnitNumber()` (counting `IsMilitaryLandUnitDelegate`, cached in the new `PlayerCache.CurrentAllLandMilitaryUnitNumber`) joins the existing `getCurrentMilitaryUnitNumber()` (`IsLatestMilitaryLandUnitDelegate`), backed by a new `isMilitaryLandUnit(Unit)` overload. `getFortValue` and `calculateEffectCityValue` switch to the all-land count outright; `calculateUnitValueModifier`, `calculateTechValue` and `isBuildUnitValid` pick between the two on `isLatestMilitaryLandUnit(eUnit)`, `upgradeValue` narrows to latest-tech only, and `isBuildUnitValid` drops `countUnits(IsEmergencyUnitDelegate)` from its headcount. This is the mechanism behind "AI builds fewer Militia, Conscripts and obsolete units". Note `PlayerCache.clear()` resets `CurrentMilitaryUnitNumber` and `OrderSurplus` but not `CurrentAllLandMilitaryUnitNumber`
- **Build queue gains an explicit low-priority tier** — `PlayerAI.BuildValue` adds a `bool bLowPriority` field sorted ahead of `iValue` in `CompareTo`; in `getBestBuild`, projects and specialists set it false while unit entries set it from `isEmergencyUnit(eLoopUnit)` or the relevant headcount exceeding `getTargetMilitaryUnitNumber()`
- **New order-surplus budget gates AI expansion attacks** — `PlayerAI` adds `calculateOrderSurplus()` (`ORDERS_YIELD` minus one per alive worker), the cached `getOrderSurplus()` with its own `getOrderSurplusLock` and `PlayerCache.OrderSurplus`, and `hasMilitaryOrders()` returning `getOrderSurplus() >= getCurrentAllLandMilitaryUnitNumber()`. Its sole consumer is `UnitRoleManager.assignUnitRoles`, whose "expansion targets are highest priority" branch changes from `if (mbMilitaryReadyToDefend)` to `if (mbMilitaryReadyToDefend && AI.hasMilitaryOrders())`
- **AI always guards city sites, even having met nobody** — `UnitRoleManager.assignCitySiteGuardUnits` drops its early-`continue` for `eRole == Unit.RoleType.CLAIM_TILE && !AI.isTeamContactAny() && AI.getTileDanger(pTarget) == 0 && pTarget.hasImprovement()` (and the nested `getImprovementDevelopTurns` test). Relatedly, `PlayerAI.isDesirableCitySite(Tile)` widens `protected virtual`→`public virtual` so `UnitAI.isAvoidMove` can append `|| AI.isDesirableCitySite(unit.tile())`
- **Units moved by an ally cannot act again that turn** — new AI flag `mbAllyMoved`: `Unit.setTileID` calls `AI.setAllyMoved(true)` when `pActingPlayer != null && !bOwnerActing && game().areAllied(...)`, and `Unit.processEndTurn` clears it. New `Unit.UnitAI.canLeaveTile()` returns false while `isAllyMoved()`, and both `doMoveTribe` and `doActionInPlace` now call it instead of `unit.canLeaveTile(unit.tile())`. Plumbed through `DirtyType.mbAllyMoved`, `NetworkData.mbAllyMoved`, `dirtyValuesIO`, and an `"AllyMoved"` element in `writeGameXML`/`readGameXML`
- **Tribal units retreat to allied city territory, not just their own tribe's cities** — `Unit.UnitAI.doTribeMoveBackHome` changes its home test from `pLoopTile.hasCity() && (pLoopTile.city().getTribe() == unit.getTribe())` to `pLoopTile.hasCityTerritory() && game.isCityAllied(pLoopTile.cityTerritory(), getTeam(), unit.getTribe(), true)`
- **AI move targets exclude tiles it should leave for another player** — `Unit.UnitAI.isValidMoveTarget` adds `&& !AI.shouldLeaveTileForOtherPlayer(pTile)` to its `canUnitOccupy` branch
- **Attack and approach paths record their chosen move tile** — `Unit.UnitAI.killTarget` and `moveTowardsTarget` both add `setMoveTile(pMoveTile.getID())` after selecting `pMoveTile`; `killTarget` also passes `ValidMoveTargetDelegate` (was `null`) as `getBestAttackTile`'s `tileCondition`
- **`ClosestSettlement` cache dropped from `Unit.UnitAI`** — the `mpClosestSettlement` backing field, the `ClosestSettlement` property, and its reset in `clearTempData` are removed; `Tribe.findClosestSettlement` now has no callers in `Reference/Source`

### Maps & map generation

- **Mirrored maps lock an outer water margin during land generation** — `MapScriptContinent`, `MapScriptContinents`, `MapScriptDonut` and `MapScriptHardwoodForest` each add an `if (MirrorMap)` block to the `MakeWater` loop in `GenerateLand`, calling `LockTileTerrain(tile, WATER_TERRAIN)` for tiles outside `xStart`/`yStart`..`MapWidth - xStart`/`MapHeight - yStart`. `MapScriptHardwoodForest.GenerateLand` additionally hoists `xStart`/`yStart` to method entry as `MAP_BOUNDARY_IMMUTABLE_OUTER_WIDTH + 2` and restates `LandFrac.initContained` and both loop bounds as `MapWidth - 2 * xStart` / `MapHeight - 2 * yStart` (arithmetically equivalent to the old `iBoundary * 2 + 4` form)
- **Point-mirrored Arid Plateau and Coastal Rain Basin fall back to base mountain placement** — `AridPlateau.AddMountains` and `CoastalRainBasin.AddMountains` each gain an early `if (!mountainsTop && !mountainsBottom) { base.AddMountains(iCoreAngle); return; }` before the mountain-elimination counting logic. Both scripts only clear those flags on the centerpoint-symmetric path, so this is the point-mirrored case — the fix for too few mountains and the strange river generation that followed
- **Bonus improvement placement picks an improvement valid for the tile's terrain** — `DefaultMapScript.AddBonusImprovements` no longer takes `bonusImprovementsToUse[0]` unconditionally; it walks the candidate list testing each `maeTerrainValid` entry with `tile.isTerrainTarget(...)` and keeps the first match, `continue`-ing when none matches. The downstream `maeExplorationTexts`/`mbUrban`/`mbNoVegetation` reads all go through that local

### Events & characters

- **Life-event announcements cut back to leader and heir** — `Character.shouldAnnounce` is rewritten from a broad test (`isLeaderOrSpouseOrSuccessor`, `isCourtier`, `isReligionHead`, `isFamilyHead`, `isClergy`, `isOrWasRegent`, plus a per-`RatingType` scan of `maiRating`/`maiPermanentRating` and a `getTraitDieProb` fallback) down to `isLeader() || isHeir()`. The signature gains a `ReligionType eReligion = ReligionType.NONE` parameter; when a religion is passed the test widens to `isLeader() || isHeir() || isFamilyHead()`, and `Character.chooseReligion` now calls `shouldAnnounce(TraitType.NONE, eBestReligion)`. Note `eTrait` is now accepted but unused, so the existing `shouldAnnounce(eTrait)` call sites all collapse to the leader/heir test
- **Character-death achievements now check ownership** — `Player.doBonus` (`PlayerBonus.cs`) moves both achievement checks inside a new `if (pCharacter.player() != null)` guard; `meAchievementIfHeir` additionally requires `pCharacter.player() == this` and `meAchievementIfOtherLeader` requires `pCharacter.player() != this` (previously just `isLeader()`, so a player's own leader could award the "other leader" achievement). `bonus.xml`: `BONUS_KILL_CHARACTER_MURDERED` gains `AchievementIfHeir ACHIEVEMENT_ASSASSINATED_HEIR` and `AchievementIfOtherLeader ACHIEVEMENT_ASSASSINATED_ANOTHER_LEADER`
- **Ancient Ruins exploration is now per-improvement data instead of hardcoded globals** — `Tile.doImprovementBonus` replaces `infos().Globals.RUINS_EXPLORED_STAT` and `RUINS_EXPLORED_EVENTTRIGGER` with `infos().improvement(eOriginalImprovement).meExplorationStat` (guarded on `!= StatType.NONE`, now applied after `pUnit.doRuinsXP`) and `.meExplorationTrigger`. `improvement.xml`: `IMPROVEMENT_ANCIENT_RUINS` `ExplorationStat (absent)→STAT_RUINS_EXPLORED`, `ExplorationTrigger (absent)→EVENTTRIGGER_RUINS_EXPLORED`
- **Cutting vegetation fires its trigger from the acting-player overload too** — `Unit.clearVegetation(Player, List<TileText>)` caches `originalVegetationInfo` before `pTile.clearVegetation(...)` and then fires `VEGETATION_CUT_EVENTTRIGGER` when `hasPlayer()`; the sibling overload already did this, so the second call site is brought in line
- **Schemer and Scholar base ratings swapped** — `trait.xml`: `TRAIT_SCHEMER_ARCHETYPE` drops `aiRating.RATING_COURAGE -1` and takes `RATING_WISDOM 4→3`; `TRAIT_SCHOLAR_ARCHETYPE` gains `RATING_COURAGE (absent)→-1` and `RATING_WISDOM 3→4`
- **The Gatherer and The Resourceful cognomens made cheaper and worth less** — `cognomen.xml`: `COGNOMEN_GATHERER` `iLegitimacy 30→20`, `iMinValue 3000→2000`; `COGNOMEN_RESOURCEFUL` `iLegitimacy 50→40`, `iMinValue 5000→4000`; both `aiStatValue.STAT_RESOURCE_HARVESTED 300→200` and `STAT_VEGETATION_REMOVED 100→50` (Empires of the Indus)

### UI

- **New "Pillaged" filter for Improvements in Map Search** — `ClientUI.MapImprovementSearchType` gains a `PILLAGED` member and `ClientUI.getMapSearchResults` a matching case calling `checkAddImprovements(tile => tile.isPillaged() && ...)`; `text-new.xml` adds `TEXT_UI_MAPSEARCH_IMPROVEMENT_PILLAGED` ("Pillaged")
- **Map Search result cycling now selects and acts on the highlighted entry** — `ClientUI.doWidgetAction`'s `ItemType.MAP_SEARCH_CYCLE` case sets `mMapSearch.SetInt("SelectedEntry", miMapSearchSelectionIndex)` and re-dispatches that entry's own widget action from its `-Entry` subtag; five sibling cases (`MAP_SEARCH`, `_TOGGLE`, `_PLAYER`, `_TYPE`, and the `updateMapSearchDropdowns` case) each reset `SelectedEntry` to `-1`
- **Improvement adjacency whitelist surfaced in help from both directions** — `HelpText.buildImprovementHelp` lists an improvement's `maeAdjacentImprovementValid` entries under the new `TEXT_HELPTEXT_IMPROVEMENT_VALID_ADJACENT_IMPROVEMENTS` ("Allows {LIST,2} to be built on adjacent tiles", new in `text-new.xml`); `buildImprovementRequiresHelp` scans all improvements for the reverse containment and adds them to the or-list — reusing `TEXT_HELPTEXT_IMPROVEMENT_REQUIRES_ADJACENT_HEIGHT`, which reads as a copy-paste of the height requirement string rather than a purpose-built one. `ClientUI.GetValidImprovementsForTile` now passes `bTestEnabled: pTile.canHaveImprovementAdjacentTest(...)`, so improvements blocked purely by adjacency grey out rather than disappearing
- **Make Urban tooltip rebuilt around a real requirements list** — `ClientUI.populateImprovementTooltip`'s `WorkerActionType.URBAN` branch drops the `HelpText.buildUrbanHelp` blurb (`Desc-Label` is now `""`) and instead populates a `requirementsScope` with a `TEXT_HELPTEXT_REQUIRES` warning per `mbAddUrban` effect player when `!isAddUrbanUnlock()`, rendered through `Requirements-Count`/`-IsActive`. `HelpText.buildAddUrbanText` is called with a `" "` separator and now also adds `addYieldCost(ORDERS_YIELD, UNIT_ADD_URBAN_COST, true)`
- **Zero-cost buy lines suppressed in road and urban tooltips** — both branches hoist `int iCost` and guard `HelpText.buildBuyYieldsKeyText(...)` behind `if (iCost > 0)`; the trailing `bDiffs` argument flips `true`→`false` on the road path and `false`→`true` on the urban path, and the urban path now uses the return of `getAddUrbanCost(...)` rather than `getBuyYieldsCost` on the filled dictionary
- **Legitimacy help links are bound to a specific player** — `HelpText.buildLegitimacyLinkVariable` gains a leading `Player pPlayer` parameter (and `buildLegitimacyBonusLinkVariable(Player)`), appending `pPlayer?.getPlayerInt().ToStringCached()` as widget data; `buildLinkHelp` resolves `pGame?.player(ePlayer) ?? pActivePlayer` instead of always using the active player. Call sites updated across `HelpText.Bonus.cs`, `HelpText.Effect.cs`, `HelpText.Game.cs`, `HelpText.Widget.cs`, `HelpText.cs`, `PlayerBonus.cs` and `ClientUI.updateUnitActions`
- **Unit-consumption links carry their owning player** — `HelpText.buildUnitConsumptionLinkVariable(YieldType, Player)` and `buildUnitConsumptionOutsideLinkVariable(Player)` likewise append the player as widget data, and `buildLinkHelp` iterates `pGame.player(ePlayer).unitAt(i)` instead of `pActivePlayer.unitAt(i)` — the fix for the breakdown always showing the active player's units
- **Reign span now shown for former leaders** — `HelpText.buildCharacterHelp` changes the guard around `buildLeaderReignVariable(pCharacter)` from `pCharacter.isLeader()` to `pCharacter.isOrWasLeader()`
- **Unit build tooltips get an explicit title line** — `HelpText.buildWidgetHelp`'s `ItemType.BUILD_UNIT` case opens a `buildTitleScope` with `buildUnitTypeLinkVariable(eUnit, pGame)` and passes `bName: false` to `buildUnitTypeHelp`
- **City-effect source links prefer religion attribution** — `HelpText.buildEffectCitySourceLinkVariable` moves its `getNumSources() > 1` multi-source branch to after the `meSourceOpinionReligion`/`meSourceReligion` branch that emits `TEXT_HELPTEXT_LINK_EFFECT_CITY_SOURCE_RELIGION_TYPE`
- **Minimap ping placement scales to the render-texture rect** — `Minimap.UpdatePings` replaces the hardcoded `* 250f` on both axes with `* renderTextureRect.rect.width` / `.height` (decompiled)
- **Timeline hover reads the year under the cursor** — `HistoryPopup.OnWidgetHover`'s `ItemType.TIMELINE_YEAR` case converts `Input.mousePosition` through `RectTransformUtility.ScreenPointToLocalPointInRectangle`, derives a normalized fraction across `Timeline.RectTr`, and calls `buildYearTextVariable((int)(num * mGame.getTurn()) + 1, mGame)` instead of using the shown turn. Note the added `Mathf.Clamp(num, 0f, 0.9999f);` is a bare expression statement whose result is discarded, so it does not actually constrain the value used (decompiled)
- **Null-city guards added to help and player counters** — six `HelpText.City.cs` link builders (`buildCityLinkVariable`, `buildCityCaptureLinkVariable`, `buildCityImprovementsLinkVariable`, `buildCityLuxuriesLinkVariable`, `buildCityProjectsLinkVariable`, `buildCityStrengthLinkVariable`) gain an `if (pCity == null) { return TEXTVAR(false); }` prologue (`buildCitySpecialistsLinkVariable` is left unguarded), and twelve `Player.cs` city-loop counters — `countCultureCities`, `countCultureStepVPs`, `countSpecialists`, `countSpecialistClasses`, `countProjects`, `countEffectCities`, `countFamilyCities`, `countFamilySpecialists`, `countReligionCities`, `hasIdleCity`, `isCityYieldBuildCurrent`, `countCityBuildProject` — now null-check `game().city(iCityID)` before dereferencing it
- **Ambition/Legacy help string localized** — `text-change.xml` adds `TEXT_HELPTEXT_LINK_HELP_LEGACY` ("Complete ten Ambitions and Legacies to win an Ambition Victory…") across all eleven shipped languages
- **Unlimber audio moved from the button to the action** — `ClientUI.PopulateButtonsForTile` drops `buttonData.AudioClickOverride = "AUDIO_UI_UNLIMBER"` and `Unit.unlimber` sets `unitAction.mzSound = "AUDIO_UI_UNLIMBER"` on the `UnitUnlimberAction` instead, so the sound no longer overrides subsequent selections

### Modding

- **Four new `InfoImprovement` fields** — `InfoBase.cs` adds `meExplorationTrigger` (`ExplorationTrigger`), `meExplorationStat` (`ExplorationStat`), `mbHideUnits` (`bHideUnits`) and `maeAdjacentImprovementValid` (`AdjacentImprovementValid`), all four declared in the `improvement.xml` schema entry. Only the two exploration fields carry shipped values (on `IMPROVEMENT_ANCIENT_RUINS`); `bHideUnits` and `AdjacentImprovementValid` are wired up in engine and help text but unused by shipped content
- **New permissive adjacency rule** — `Tile.isImprovementValid` gains a `DirectionType` loop inside its `bTestTerrain` block: any adjacent tile that `hasActiveImprovement()` and whose `improvement().maeAdjacentImprovementValid.Contains(eImprovement)` makes the tile valid. This is a whitelist that *grants* validity, distinct from the `mbNoAdjacent` restriction used for Forts
- **`Tile.canHaveImprovementAdjacentTest` widened `protected virtual`→`public virtual`** so the client can pre-test adjacency; `PlayerAI.isDesirableCitySite` likewise widened to `public virtual`
- **`InfoImprovement.meExplorationTexts` renamed `maeExplorationTexts`** — naming-convention fix for the list prefix; the XML tag `ExplorationTexts` is unchanged. Call site updated in `DefaultMapScript.AddBonusImprovements`
- **`Tribe.processEndTurn` removed** — the `public virtual void processEndTurn()` that reverse-iterated `getUnits()` calling `Unit.processEndTurn()` is deleted with no replacement and had no remaining caller; the equivalent work is done by the `pLoopUnit.isTribe()` loop in `Game.cs`, unchanged this update. Relevant to anyone Harmony-patching that target

### Client & infrastructure

- **Temporary Mac crash debug harness, gated to developer machines** — `BuildInfo.cs` gains a 182-line `#region TEMP DEBUG - machine-targeted logging for a Mac crash` block: `IsTargetDebugMachine` (cached; matches `SystemInfo.deviceName`/`Environment.UserName` against hardcoded `DEBUG_MACHINE_DEVICE = "KJS MacBook Pro"`, `DEBUG_MACHINE_USER = "Genesis"`, `DEBUG_MACHINE_LINUX_USERS = { "solver", "edd" }`, or a `-debugmachine` command-line arg), plus `LogMachineIdentity()` and the `IsTargetMac`/`IsTargetLinuxUser`/`GetDeviceName`/`NormalizeMachineName` helpers. Its consumer is a new RectTransform-recursion guard in `Mohawk.UISystem.BaseUI` — `EnterRectChange`/`ExitRectChange`/`ReportRectChange`/`GetHierarchyPath` with `RECT_CHANGE_MAX_DEPTH = 32`, `RECT_CHANGE_MAX_PER_FRAME = 512`, `RECT_CHANGE_MAX_REPORTS = 100` — which both `BaseUI.OnRectTransformDimensionsChange` and `MohawkContentSizeFitter.OnRectTransformDimensionsChange` now enter, wrapping their bodies in `try`/`finally`. `EnterRectChange` returns false immediately unless `BuildInfo.IsTargetDebugMachine`, so the guard is inert on retail machines (decompiled Assembly-CSharp plus `Reference/Source` `BuildInfo.cs`)
- **UI raycasting hardened against destroyed Graphics** — `MohawkPointerInputModule` adds `SafeRaycastAll(PointerEventData, List<RaycastResult>)` and `static PurgeDestroyedGraphics()`, which reflects into `GraphicRegistry`'s private `m_Graphics`/`m_RaycastableGraphics` dictionaries (cached in `graphicRegistryFields`) and removes entries whose `canvasRenderer` is null. Both `GetTouchPointerEventData` and `GetMousePointerEventData` switch from `eventSystem.RaycastAll(...)` to `SafeRaycastAll(...)` (decompiled)
- **Popup and canvas lifecycle fixes** — `Popup.OnDisable` now chains `base.OnDisable()`, which stops an in-flight tween via `UITweens.DoTween(this, UITweens.StopID)` (previously never run when a popup was disabled); `BaseUI.OnEnable` adds `UpdateForVisibilityChange()` after `UpdateRect()`; and `UICanvas.UnregisterCanvas` splits its combined condition so `isCanvasRegistered = false` runs even when `_canvas` or `uiElement?.UIManager` is already null (decompiled)
